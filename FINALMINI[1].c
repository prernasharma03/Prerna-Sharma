...
int main()
{
    ...
    printf("|||                                                                                                                                                                                                     |||\n");
    printf("|||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||\n");
    printf("                                       |||   we_care_for_your_privacy    |||                                     \n");
...
system("color 7A");
    ...
    char loop[10];
do{
   system ("espeak -ven-us welcome"); 
   char phrase[100]="WE CARE FOR YOUR PRIVACY";
   char charcommand[100];
   strcpy(charcommand,"espeak -ven-us \"");
...
// choose operation----ENCRYPTION OR DECRYPTION------
  ...
  int temp1,x;
  int b1,p;char ch1;
  char phrase[100]="ENTER YOUR INPUT";
...
// LETTER INTERCHANGE
    ...
    char p2[100]="Encryption successful";
//char charcommand[100];
    strcpy(charcommand,"espeak -ven-us \"");
    strcat(charcommand,p2);
    strcat(charcommand,"\"");
    system(charcommand);
    ...
    //MESSAGING PART-------
    printf("\nDo you want to send the encrypted message? (YES/NO)\n");
    char ans[10]; int f;
    scanf("%s",ans);
...
if(spassword[0]=='1' && spassword[1]=='2' && spassword[2]=='3' && spassword[3]=='4'){
    p=1;
} else{
    p=0;
}
...
char p1[100]="COPY PASTE THE ENCRYPTED TEXT\n";
//char charcommand[100];
    strcpy(charcommand,"espeak -ven-us \"");
    strcat(charcommand,p1);
    strcat(charcommand,"\"");
    system(charcommand);
    Beep(800,500);
    printf("\n\nWHICH APP DO YOU WANT TO OPEN?\n");
...
for(i=0;i<strlen(b);i++){
   ...
   printf("%c",x2[i]);
    ...
    char p2[100]="Decryption successful";
//char charcommand[100];
    strcpy(charcommand,"espeak -ven-us \"");
    strcat(charcommand,p2);
    strcat(charcommand,"\"");
